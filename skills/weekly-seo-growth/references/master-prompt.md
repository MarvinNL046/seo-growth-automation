...
PROCESS
1. Run preflight and the cadence guard (in-flight collision, then the runsPerWeek budget counted over this ISO week's completed `routine` rows only; every completed row when the register has no `trigger` column; one run per ISO week when the profile declares no cadence).
